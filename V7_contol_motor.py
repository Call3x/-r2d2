...
pwm_motor1_backwards = GPIO.PWM(27, 100) # Fréquence de 100 Hz
pwm_motor2_backwards = GPIO.PWM(23, 100) # Fréquence de 100 Hz

# Démarrer les PWM avec une valeur de 0
pwm_motor1_forward.start(0)
pwm_motor2_forward.start(0)
pwm_motor1_backwards.start(0)
pwm_motor2_backwards.start(0)
...
def control_motor(x_axis, y_axis):
    if not motors_enabled:
        ...
        pwm_motor1_backwards.ChangeDutyCycle(0)
        pwm_motor2_backwards.ChangeDutyCycle(0)
    ...
    elif y_axis > 0.1:  # Joystick poussé vers l'arriere
        pwm_motor1_backwards.ChangeDutyCycle(int(abs(y_axis)*100))
        pwm_motor2_backwards.ChangeDutyCycle(int(abs(y_axis)*100))
...
try:
    ...
    pwm_motor1_backwards.stop()
    pwm_motor2_backwards.stop()
